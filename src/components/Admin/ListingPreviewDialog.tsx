...
export function ListingPreviewDialog({ listing, onClose, onApprove, onReject }: ListingPreviewProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-2xl animate-rmk-fade-up overflow-hidden rounded-[20px] border border-white/[0.08] bg-[#111827] shadow-2xl text-slate-100">
        <div className="flex items-center justify-between border-b border-white/[0.08] p-5">
          <h3 className="text-lg font-bold text-slate-100">Chi tiết tin đăng chờ duyệt</h3>
          <button
            onClick={onClose}
        ...
        <div className="max-h-[70vh] space-y-6 overflow-y-auto p-6">
          {(!listing.images || listing.images.length === 0) ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/[0.08] bg-[#1A2233] py-8 text-slate-400">
              ...
              <span className="text-xs font-medium">Không có ảnh đính kèm cho tin đăng này</span>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-3">
              {listing.images.map((img: any, idx: number) => (
                <a key={idx} href={img.image_url} target="_blank" rel="noreferrer" className="group relative">
                  <img
                    src={img.image_url}
                    alt={`Sản phẩm ${idx + 1}`}
          ...
          <div>
            ...
            <h4 className="mt-2.5 text-xl font-bold text-slate-100">{listing.title}</h4>
          ...
          <div className="space-y-2 rounded-xl border border-white/[0.06] bg-[#1A2233] p-4">
            <div className="text-xs font-bold uppercase tracking-wider text-slate-400">Thông tin giao dịch</div>
            <div className="mt-1 grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2">
                <ArrowLeftRight className="size-4 text-blue-400" />
                <span className="text-slate-400">Thương lượng:</span>
                <span className="font-semibold text-slate-200">
                  {listing.is_negotiable ? "Có thể thương lượng" : "Không thương lượng"}
                </span>
              </div>
              <div>
                <span className="text-slate-400">Khu vực:</span>
                <span className="ml-1.5 font-semibold text-slate-200">{listing.location_city || "Chưa cập nhật"}</span>
              </div>
            </div>
          </div>

          <div>
            <div className="mb-2 text-sm font-bold text-slate-200">Mô tả sản phẩm:</div>
